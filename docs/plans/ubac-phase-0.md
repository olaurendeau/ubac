...
E1 (squelette), E2 (garde-fous structurels) et E3 (types marqués) sont intégrées
sur `main`. Le code présent est `src/core/types.ts`, avec tests structurels et
assertions TypeScript. Les autres lots restent à livrer et à valider.

Les anciennes E4–E24 sont remplacées par P1–P7 ci-dessous. Leurs identifiants
restent uniquement des références de traçabilité ; ne pas les dispatcher comme
tâches supplémentaires. Les sous-sections détaillées d'un lot ne sont pas des PR.

| Travail ouvert à reprendre | Nouveau lot |
|---|---|
| PR #12 — ladder, `etape/ubac-phase-0-14` | P4 |
| PR #14 — téléchargement fixture, `etape/ubac-phase-0-24` | P6 |
| PR #15 — benchmarks, `etape/ubac-phase-0-17` | P5 |

Inspecter les diffs, les retours de revue et la base de ces PR avant reprise.
Conserver leur branche et compléter le périmètre vers le lot entier ; mettre à
jour titre et description. Aucune PR ouverte n'est considérée comme un acquis.
Examiner aussi les branches non intégrées E7, E8, E11 et les références `pr*`
avant de recréer du code. Les métadonnées de worktrees Linux ne prouvent pas
l'arrêt des anciens workers : vérifier leur propriétaire avant toute reprise.
Ne pas supprimer les branches ou élaguer les worktrees pendant la migration.
...
- Première vague : P1 et P6 sont indépendants.
- Après P1 : P2, P3, P4 et P5 deviennent éligibles, sous réserve de leurs gates.
- P7 attend l'intégration de P2 à P6.

Trois workers simultanés maximum, construction et revue comprises. Chaque lot
regroupe les anciennes écritures concurrentes d'un même module (risque,
rééquilibrage, benchmarks). Les fichiers partagés ajoutés en cours de travail,
notamment `package.json`, types et configuration, restent à sérialiser par le
coordinateur ; l'isolation Git ne supprime pas les conflits d'intégration.

Sept lots restants, trois étapes acquises. La fixture ne nécessite plus une
exception de taille : ses données générées sont contrôlées séparément. Aucune
